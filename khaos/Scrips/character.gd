...
func Enemy_receber_dano(dano):
	...
	if not barra_de_vida.has_method("receber_dano"):
		return
	barra_de_vida.call("receber_dano", dano)

func morrer():
	queue_free()

func _on_soco_area_body_entered(body: Node2D) -> void:
	print("Entrou em _on_SocoArea_area_entered")
	if body.is_in_group("Player"):
		print("Colisão detectada com inimigo")
		if combo[contcombo] == "soco_direita" or combo[contcombo] == "soco_esquerda":
			print("Dano de soco aplicado: 5")
			body.call("receber_dano", 5)  
		elif combo[contcombo] == "uppercut":
			print("Dano de uppercut aplicado: 10")
			body.call("receber_dano", 10)
