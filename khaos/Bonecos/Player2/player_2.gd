...
func _physics_process(delta: float) -> void:
	...
	# Ataques
	if Input.is_action_just_pressed("p2attack") and not esta_bloqueando:
	...
	# Abaixar
	if Input.is_action_pressed("p2down") and is_on_floor() and not esta_pulando and not esta_atacando:
		esta_abaixando = true
	...
	# Bloqueio
	if Input.is_action_pressed("p2block") and is_on_floor() and not esta_pulando and not esta_atacando:
