...
func _physics_process(delta: float) -> void:
	...
	# Pulo
	if Input.is_action_just_pressed("p2jump") and is_on_floor() and not esta_atacando:
		velocity.y = FORCA_PULO
		esta_pulando = true

	# Movimento horizontal - Só permite movimento quando não está atacando
	var direcao := Input.get_axis("p2left", "p2right")
	...
	if Input.is_action_just_pressed("p2attack") and not esta_bloqueando and not esta_pulando:
		if Input.is_action_pressed("p2down") and is_on_floor():
			iniciar_ataque_baixo()
		else:
			iniciar_ataque()

	# Abaixar
	esta_abaixando = Input.is_action_pressed("p2down") and is_on_floor() and not esta_pulando and not esta_atacando
	if esta_abaixando:
		animation_player.play("crouch")

	# Bloqueio
	esta_bloqueando = Input.is_action_pressed("p2block") and is_on_floor() and not esta_pulando and not esta_atacando
	if esta_bloqueando:
...
func _on_soco_area_body_entered(body: Node2D) -> void:
	if body.is_in_group("p1"):
		if combo[contcombo] == "soco_direita" or combo[contcombo] == "soco_esquerda":
			body.call("p1_receber_dano", 5)  
		elif combo[contcombo] == "uppercut":
			body.call("p1_receber_dano", 10)

var in_hit_cooldown = false

func p2_receber_dano(dano):
